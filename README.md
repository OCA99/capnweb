...
* `AsyncGenerator`, with strict mode by default and optional buffered prefetch via `.consume()`.
...
* Calling `consume(options)` on the generator enables buffered prefetch (in-place) to reduce
